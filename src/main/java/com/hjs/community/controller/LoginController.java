...
public class LoginController {
    ...
    public void getKaptcha(HttpServletResponse response) {
        ...
        //验证码的归属
        String onwer = CommunityUtil.generateUUID();
        Cookie cookie = new Cookie("kaptchaOwner",onwer);
        cookie.setMaxAge(60);
        cookie.setPath(contextPath);
        response.addCookie(cookie);
        //将验证码存到redis
        String redisKey = RedisKeyUtil.getKaptcha(onwer);
        redisTemplate.opsForValue().set(redisKey,text,60, TimeUnit.SECONDS);
        // TODO: 2023/1/25 这里一直刷新验证码 是有可能将redis刷挂掉
    ...
    public String login(String username, String password, String code, boolean rememberme, Model model, HttpServletResponse response,@CookieValue("kaptchaOwner")String kaptchaOwner) {
        ...
        if (StringUtils.isBlank(code) || !code.equalsIgnoreCase((String) kaptcha)) {
